...
export class OrdersService {
    ...
    async startGlassCuttingJob(inputData: GlassCuttingDto): Promise<void> {
        ...
        const width = Number(inputData.width ?? material?.width);
        const height = Number(inputData.height ?? material?.height);

        if (!Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) {
            console.log('Invalid material dimensions. Job aborted.');
            return;
        }
        ...
        const packableItems = allItems.filter(
            (item) =>
        ...
        const worker = new Worker(
            path.resolve(__dirname, '../workers/glass-cutting.worker.js'),
        );
        ...
        worker.on('message', async (result) => {
            if (result.status === 'completed') {
                ...
                try {
                    ...
                    await fs.writeFile(
                        CUTTING_RESUL_FILE_PATH,
                        JSON.stringify(data, null, 2),
                    );
                    console.log(
                        `Glass cutting saved: ${data.packedItems?.length ?? 0} packed, ` +
                            `${data.unpackedItems?.length ?? 0} unpacked, ` +
                            `utilization ${((data.utilization ?? 0) * 100).toFixed(1)}%`,
                    );
                } catch (dbError) {
                    console.log(
                        'Failed to save the cutting result.',
                        dbError.stack,
                    );
                }
            } else {
    ...
    async getMaterialGrid () {
        ...
        if (!glassCuttingData?.packedItems?.length) {
            return glassCuttingData;
        }

        glassCuttingData.packableItems = await Promise.all(
            glassCuttingData.packedItems.map(async (packItem) => {
